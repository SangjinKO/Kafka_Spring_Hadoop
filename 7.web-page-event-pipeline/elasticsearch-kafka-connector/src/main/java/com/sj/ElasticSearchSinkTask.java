...
import com.sj.config.ElasticSearchSinkConnectorConfig;
    ...
    public void put(Collection<SinkRecord> records) {
        if(records.size() > 0){
            ...
            for (SinkRecord record : records) {
                Gson gson = new Gson();
                logger.info("CHECK RECORD : {}", record.value().toString());

//                JsonReader reader = new JsonReader(new StringReader(record.value().toString()));
//                reader.setLenient(true);
//                Map map = gson.fromJson(reader, Map.class);

//                Map map = Map.of("key1","value1", "key2", "value2")

                Map map = gson.fromJson(gson.toJson(record.value()), Map.class);
